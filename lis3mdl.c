...
status_t getFullScaleConfig(lis3mdl_range_t *pFullScaleRange)
{
    ...
    *pFullScaleRange = GET_BIT(data, REG_CTRL_REG2_BIT_MASK_FS1_FS0, REG_CTRL_REG2_BIT_POS_FS1_FS0);
...
static status_t setIntThreshold(uint16_t value)
{
    ...
    status_t return_v = i2c_write(I2CADDR_DEFAULT, LIS3MDL_REG_INT_THS_L, 2, &value);
}
...
{
    uint8_t buffer[6];
    status_t return_v = i2c_read(I2CADDR_DEFAULT, LIS3MDL_REG_OUT_X_L, 6, &buffer);
    ...
    return_v &= getFullScaleConfig(fullScaleRange);
